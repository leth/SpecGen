...
# end
	...
	# danbri added another term.id 20010101 and removed term.status
       ...
       zz = eg % (cssinfo, term.id,term.uri,"rdfs:Class","Class", sn, term.id, term.label, term.comment, term.status,domainsOfClass,rangesOfClass+subClassOf+hasSubClass+classIsDefinedBy+isDisjointWith+isEquivalentTo, s)
...
# end
	...
	# danbri added another term.id 20010101
       ...
       if (term.status == "archaic"):
         cssinfo = cssinfo + 'archaic '
       zz = eg % (cssinfo, term.id, term.uri,"rdf:Property","Property", sn, term.id, term.label, term.comment,term.status,domainsOfProperty,rangesOfProperty+inverseOfProperty+propertyIsDefinedBy+ifp+fp, s)
